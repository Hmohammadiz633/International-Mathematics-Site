...
// کامپوننت داخلی ContactButtons جهت تضمین عدم برخورد با خطای Module Not Found
function IntegratedContactButtons({ lang }) {
  const isFa = lang === 'fa';
  const [formData, setFormData] = useState({ name: '', phone: '', message: '', captcha: '' });
  ...
  const handleSubmit = (e) => {
    ...
    if (parseInt(formData.captcha) !== num1 + num2) {
      setStatus(isFa ? 'پاسخ امنیتی اشتباه است!' : 'Incorrect security answer!');
      return;
    }
    setStatus(isFa ? 'پیام شما با موفقیت ثبت شد.' : 'Message sent successfully.');
    setFormData({ name: '', phone: '', message: '', captcha: '' });
  };
    ...
    <section className="max-w-5xl mx-auto px-4 pt-6">
      <div className="bg-white text-gray-900 rounded-3xl p-6 md:p-8 border border-gray-200/90 shadow-md">
        <h4 className="text-xl md:text-2xl font-black text-slate-900 mb-6 flex items-center gap-3 border-b border-gray-100 pb-3 leading-snug">
          ...
          <span>{isFa ? 'ارتباط با استاد و مشاوره' : 'Contact & Consultation'}</span>
        </h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-3">
            <a
              ...
              className="flex items-center justify-between p-4 bg-blue-50/50 hover:bg-blue-100/50 border border-blue-200 rounded-2xl transition group"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-500">@International_Maths</div>
                </div>
              </div>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-between p-4 bg-pink-50/50 hover:bg-pink-100/50 border border-pink-200 rounded-2xl transition group"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-500">@Hadi_mohammadi_zarandini</div>
                </div>
              </div>
            ...
            <a
              href="https://t.me/Maths_Admin"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-between p-4 bg-slate-100 hover:bg-slate-200/70 border border-slate-300 rounded-2xl transition group"
            >
              <div className="flex items-center gap-3">
                <span className="text-2xl">👨‍💼</span>
                <div>
                  <div className="font-bold text-slate-900 text-sm md:text-base">
                    {isFa ? 'ارتباط مستقیم با مدیریت' : 'Direct Admin Contact'}
                  </div>
                  <div className="text-xs text-slate-500">@Maths_Admin</div>
                </div>
              </div>
              <span className="text-slate-700 font-bold group-hover:translate-x-1 transition-transform">↗</span>
            </a>
          </div>

          {/* فرم ارسال پیام */}
          <form onSubmit={handleSubmit} className="bg-slate-50 p-5 rounded-2xl border border-slate-200/80 space-y-3">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-sm focus:outline-none focus:border-blue-500"
              />
            </div>

            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">
                {isFa ? 'شماره تماس یا آیدی تلگرام' : 'Phone or Telegram ID'}
              </label>
              <input
                type="text"
                required
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">
                {isFa ? `حاصل عبارت: ${num1} + ${num2} = ?` : `Solve: ${num1} + ${num2} = ?`}
              </label>
              <input
                type="number"
                required
            ...
            <button
              type="submit"
              className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl text-sm shadow transition"
            >
              {isFa ? 'ارسال پیام' : 'Send Message'}
            </button>

            {status && (
              <div className="text-xs text-center font-bold mt-2 text-blue-800">
                {status}
              </div>
            )}
          </form>
